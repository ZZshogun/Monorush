...
bool AudioFile<T>::decodeAiffFile (std::vector<uint8_t>& fileData)
{
    ...
    for (int i = 0; i < numSamplesPerChannel; i++)
    {
        for (int channel = 0; channel < numChannels; channel++)
        {
            ...
            {
                int64_t sampleAsInt = 0;
                sampleAsInt = (int64_t)(fileData[sampleIndex] << 16) | (fileData[sampleIndex + 1] << 8) | fileData[sampleIndex + 2];
...
int AudioFile<T>::getIndexOfChunk(std::vector<uint8_t>& source, const std::string& chunkHeaderID, int startIndex, Endianness endianness)
{
	...
	while (i < (size_t)source.size() - dataLen)
	{
		if (memcmp(&source[i], chunkHeaderID.data(), dataLen) == 0)
		{
			return (int)i;
		...
		i += dataLen;
		size_t chunkSize = (size_t)fourBytesToInt(source, (int)i, endianness);
		i += (dataLen + chunkSize);
	}
